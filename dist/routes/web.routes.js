...
const code_gen_1 = require("../lib/code-gen");
...
function createWebRouter(cards) {
    ...
    router.get('/', async (_req, res, next) => {
        try {
            ...
            res.render('landing', {
                ...
                description: 'Dangodeck is an Anime Trading Card Game database and professional developer API. Browse cards, fuzzy-search, and build with a fast REST API.',
                total: facets.total,
    ...
    router.get('/cards', async (_req, res, next) => {
        try {
            ...
            res.render('cards', {
                ...
                description: 'Browse the full Dangodeck anime TCG card catalog. Search, filter by element and anime, and sort.',
                facets,
                sorts: SORTS,
    ...
    router.get('/docs', (_req, res) => {
        const base = config_1.config.siteUrl;
        const endpoints = DOCS_ENDPOINTS.map((ep) => ({ ...ep, examples: (0, code_gen_1.genAll)(base, ep.path) }));
        res.render('docs', {
            page: 'docs',
            title: 'API Docs - Dangodeck',
            description: 'Dangodeck REST API documentation - endpoints, parameters, rate limits, and code examples in 6 languages.',
            base,
            endpoints,
        });
    });
    ...
    router.get('/cards/:id', async (req, res, next) => {
        try {
            ...
            res.render('card-detail', {
                ...
                related,
                max,
                statConfig: stats_1.STAT_CONFIG,
            });
        }
        catch (err) {
            next(err);
